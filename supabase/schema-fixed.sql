...
-- 順序: schema-fixed.sql -> seed.sql
-- （修正：is_admin 移到 profiles 建表後，避免 42P01）
-- =============================================

-- 0. 擴充
create extension if not exists "pgcrypto";

-- 1. 輔助函式（注意：is_admin 已移到 profiles 建表後建立，否則會報 42P01）
...
-- 1b. 是否為管理員（profiles 建表後才能建立，SECURITY DEFINER 避開 RLS 遞迴）
...
-- 1c. 註冊自動建 profiles（否則 /login 註冊後 Table Editor 找不到人）
